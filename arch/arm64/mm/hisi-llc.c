...
struct llc_cfg_ops {
	int (*read_one)(struct device_node *node, u32 offset,
				int bank);
...
static int hip05_read_one(struct device_node *node, u32 offset, int bank)
{
	u32 val;
	int ret, chain_id;
...
static int hi1382_read_one(struct device_node *node, u32 offset, int bank)
{
	u32 val;
	int ret, mod_id;
