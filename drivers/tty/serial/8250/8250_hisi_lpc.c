...
void __init hisi_early_lpc_out(unsigned char __iomem *lpc_base,
...
u8 __init hisi_early_lpc_in(unsigned char __iomem *lpc_base,
			    unsigned long addr)
{
	unsigned int retry;
	unsigned int lpc_cmd_value;
...
u8 __init hisilpc_8250_early_in(const struct uart_port *port,
				unsigned int offset)
{
	return hisi_early_lpc_in(port->membase,
				 (unsigned long)(port->iobase + offset));
}

void __init hisilpc_8250_early_out(const struct uart_port *port,
				   unsigned int offset, int value)
{
	hisi_early_lpc_out(port->membase, (u8)value,
			   (unsigned long)(port->iobase + offset));
}

#define BOTH_EMPTY (UART_LSR_TEMT | UART_LSR_THRE)

static void __init wait_for_xmitr(const struct uart_port *port)
{
	unsigned int status;
					    ...
					    const char *s, unsigned int count)
{
	...
	if (ier)
		hisilpc_8250_early_out(port, UART_IER, (int)ier);
}
					  ...
					  const char *options)
{
	...
	if (kstrtou32(device->options, 0, &device->baud) < 0)
		return -EINVAL;
	/*we assume that baudrate must be specified,
	...
	*/
	...
	ret = kstrtoul(p, 0, &device->port.iobase);
	if (ret)
		return ret;
...
static bool hisi_lpc_uart_disabled;
...
static int __init hisi_lpc_uart_disable(char *str)
{
	if (!strcmp(str, "off"))
		hisi_lpc_uart_disabled = true;
	else
		hisi_lpc_uart_disabled = false;

	return 1;
}
__setup("hisi_lpc_uart=", hisi_lpc_uart_disable);
