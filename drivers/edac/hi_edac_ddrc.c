...
#define ECC_MERR_MASK			BIT(4)
#define ECC_SERR_MASK			BIT(0)
...
static void hi_edac_mc_ce_handle(struct mem_ctl_info *mci, void __iomem *addr,
...
static void hi_edac_mc_ue_handle(struct mem_ctl_info *mci,
	void __iomem *addr,
...
irqreturn_t hi_edac_mc_isr(int irq, void *dev_id)
{
	...
	/* mask interruption */
	writel(0xffffff, res_addr + RASC_INTMSK);
	writel(0xffffffff, res_addr + DDRC_INTMSK);
	...
	if (DDRC_LOG_ONLY_ERR(ddrc_insts) || RASC_LOG_ONLY_ERR(rasc_insts)) {
		...
		hi_edac_mc_ce_handle(mci, res_addr, ddrc_insts, rasc_insts);
	}
	...
	if (DDRC_LOG_OS_ERR(ddrc_insts) || RASC_LOG_OS_ERR(rasc_insts)) {
		trace_frqlimit(50*HZ,
			"channel_id:0x%x,ddrc_insts:0x%x,rasc_insts:0x%x\n",
			priv->channel_id, ddrc_insts, rasc_insts);
		hi_edac_mc_ue_handle(mci, res_addr, ddrc_insts, rasc_insts);
	}

	/* clear interruption */
	writel(rasc_insts, res_addr + RASC_RINT_ADDR);
	writel(ddrc_insts, res_addr + DDRC_RINT_ADDR);
	writel(0x3000, res_addr + RASC_INTMSK);
	/* mask the CA Parity error of DDRC */
	writel(0x100, res_addr + DDRC_INTMSK);
...
static int hi_edac_mc_add(struct his_edac_ddrc *edac)
{
	struct mem_ctl_info *mci;
	...
	if (!edac) {
		dev_err(edac->dev, "hi_edac_mc_add edac was NULL\n");
		return -ENOENT;
	}
...
static int hi_ddrc_probe(struct platform_device *pdev)
{
	struct his_edac_ddrc *edac;
	...
	if (hi_edac_mc_add(edac)) {
		dev_err(&pdev->dev, "hi_edac_mc_add fail\n");
		return -ENOENT;
	}
	...
	/* mask the CA Parity error of DDRC */
	writel(0x100, edac->ddrc_base + DDRC_INTMSK);
	...
	for (count = 0; count < DDRC_IRQ_NUM; count++) {
		...
		rc = devm_request_irq(&pdev->dev, edac->irq[count],
					hi_edac_mc_isr, 0, dev_name(&pdev->dev),
					edac->mci);
		if (rc) {
...
static int hi_ddrc_remove(struct platform_device *pdev)
{
	struct his_edac_ddrc *edac = platform_get_drvdata(pdev);
