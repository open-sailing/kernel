		...
		if (SAS_ADDR(dev->sas_addr) == SAS_ADDR(sas_addr)) {
			...
			if (test_bit(SAS_DEV_GONE, &dev->state)
			  ...
			  || test_bit(SAS_DEV_DESTROY, &dev->state))
				continue;
			SAS_WARN("disco_list has the same addr: %016llx\n", SAS_ADDR(sas_addr));
			return 1;
		}
	}
...
static int sas_ex_discover_dev(struct domain_device *dev, int phy_id)
{
	...
	if (!SAS_ADDR(ex_phy->attached_sas_addr))
		return 0;
...
static int sas_check_level_subtractive_boundary(struct domain_device *dev)
{
	...
	list_for_each_entry(child, &ex->children, siblings) {
		if (test_bit(SAS_DEV_GONE, &child->state)
		  || test_bit(SAS_DEV_PROBE_FAIL, &child->state)
		  || test_bit(SAS_DEV_DESTROY, &child->state))
			continue;
...
static int sas_get_phy_attached_dev(struct domain_device *dev, int phy_id,
				    u8 *sas_addr, enum sas_device_type *type, enum sas_linkrate *linkrate)
{
	int res;
	struct smp_resp *disc_resp;
	struct discover_resp *dr;
			      ...
			      struct domain_device **src_dev)
{
	...
	list_for_each_entry(ch, &ex->children, siblings) {
		if (test_bit(SAS_DEV_GONE, &ch->state)
		  || test_bit(SAS_DEV_PROBE_FAIL, &ch->state)
		  || test_bit(SAS_DEV_DESTROY, &ch->state))
			continue;
...
static int sas_rediscover_dev(struct domain_device *dev, int phy_id, bool last)
{
	...
	memset(sas_addr, 0, 8);
	res = sas_get_phy_attached_dev(dev, phy_id, sas_addr, &type, &linkrate);
	switch (res) {
	case SMP_RESP_NO_PHY:
		phy->phy_state = PHY_NOT_PRESENT;
	...
	if ((SAS_ADDR(sas_addr) == 0) || (res == -ECOMM)) {
		...
		if (phy->linkrate != linkrate) {
			sas_unregister_devs_sas_addr(dev, phy_id, last);
			return sas_discover_new(dev, phy_id);
		}
		sas_ex_phy_discover(dev, phy_id);
		if (!SAS_ADDR(phy->attached_sas_addr)) {
			SAS_WARN("ex %016llx phy 0x%x broadcast flutter but sas_addr is 0\n",
			         SAS_ADDR(dev->sas_addr), phy_id);
			sas_unregister_devs_sas_addr(dev, phy_id, last);
			return res;
		}
