...
#define PHYCTRL_RESET_PROBLRM_OFF 7
#define PHYCTRL_DWS_LOST_OFF 10
#define PHYCTRL_RX_DISP_ERR_OFF 28
#define PHYCTRL_RX_INVLD_DW_OFF 30
			...
			u32 bit, u32 val)
{
	...
	hilink_djtag_rw(alg_hccs_subctrl_base,
		HILINK_DJTAG_SEL + macro,
		RX_CSR(lane, reg), &origin_value, 0);
	pr_info("%s %d: macro:%d, lane:%d, reg origin value = 0x%x, write value = 0x%x\n",
...
static struct hisi_sas_device *alloc_dev_v2_hw(struct domain_device *device)
{
	...
	for (i = 0; i < HISI_SAS_MAX_DEVICES; i++) {
		...
		if (hisi_hba->devices[i].dev_type == SAS_PHY_UNUSED) {
			dev_info(dev, "%s sata_dev=%d device_id=%d\n", __func__,
				sata_dev, i);
...
static void init_reg_v2_hw(struct hisi_hba *hisi_hba)
{
	...
	for (i = 0; i < hisi_hba->n_phy; i++) {
		...
		if (!get_macro_lane(hisi_hba->core_id, i, &macro, &lane))
			...
			dev_info(dev, "%s get_macro_lane fail macro: %d lane: %d\n",
				__func__, macro, lane);
...
static void phy_stp_link_accept_chk_v2_hw(struct hisi_hba *hisi_hba)
{
	...
	for (phy_no = 0; phy_no < hisi_hba->n_phy; phy_no++) {
		/* check if there is command ever delivered through this phy */
			       ...
			       struct hisi_sas_slot *slot, int abort)
{
	...
	if ((complete_hdr->dw0 & CMPLT_HDR_ERX_MSK) ||
		...
		if (SAM_STAT_GOOD != ts->stat) {
			...
			dev_info(dev, "cq: iptt:%d, task:%p, cmpl_status:%d, err_rcrd_xfrd:%d,rspns_xfrd:%d,error_phase:%d,devid:%d,io_cfg_err_code:%d,cfg_err_code:%d\n",
				slot->idx, task,
			...
			dev_info(dev, "iptt:%d dw0:0x%x,dw1:0x%x,dw2:0x%x,dw3:0x%x\n",
				slot->idx, error_info[0], error_info[1],
				error_info[2], error_info[3]);
			dev_info(dev, "cmd_table:iptt:%d, dw0:0x%x, dw1:0x%x, dw2:0x%x, dw3:0x%x, dw4:0x%x\n",
				slot->idx, cmd_table[0], cmd_table[1],
				cmd_table[2], cmd_table[3], cmd_table[4]);
...
static void phy_err_code_process_v2_hw(struct hisi_hba *hisi_hba,
	int phy_no, u32 irq_val)
{
	u32 err4_reg_val = 0, err6_reg_val = 0, reg_msk = 0;
	struct device *dev = &hisi_hba->pdev->dev;
	struct sas_phy *sas_phy = hisi_hba->phy[phy_no].sas_phy.phy;

	reg_msk = hisi_sas_phy_read32(hisi_hba, phy_no, CHL_INT2_MSK);
	hisi_sas_phy_write32(hisi_hba, phy_no, CHL_INT2_MSK,
		reg_msk | BIT(PHYCTRL_RESET_PROBLRM_OFF)
		| BIT(PHYCTRL_DWS_LOST_OFF)
		| BIT(PHYCTRL_RX_DISP_ERR_OFF)
		| BIT(PHYCTRL_RX_INVLD_DW_OFF));

	/* loss dword syn, phy reset problem*/
	if ((irq_val & BIT(PHYCTRL_DWS_LOST_OFF))
		|| (irq_val & BIT(PHYCTRL_RESET_PROBLRM_OFF)))
		err4_reg_val = hisi_sas_phy_read32(hisi_hba,
			phy_no, SAS_ERR_CNT4_REG);

	/* disparity err, invalid dword */
	if ((irq_val & BIT(PHYCTRL_RX_DISP_ERR_OFF))
		|| (irq_val & BIT(PHYCTRL_RX_INVLD_DW_OFF)))
	...
	if (err4_reg_val || err6_reg_val) {
		...
		dev_info(dev, "phy id:0x%x,loss_of_dword_sync_count:%d invalid_dword_count:%d,phy_reset_problem_count:%d,running_disparity_error_count:%d\n",
			phy_no,
			sas_phy->loss_of_dword_sync_count,
	...
	hisi_sas_phy_write32(hisi_hba, phy_no, CHL_INT2_MSK, reg_msk);
}
...
static int mul_bit_ecc_err_process_v2_hw(struct hisi_hba *hisi_hba, u32 irq_val)
{
	...
	if (irq_val & BIT(ECC_DQ_RAM_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_dqe_ram_mul_bit_ECC_err found! RAM Address is 0x%08X\n",
	...
	if (irq_val & BIT(ECC_IOST_RAM_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_iost_ram_mul_bit_ECC_err found! RAM Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_ITCT_RAM_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_itct_ram_mul_bit_ECC_err found!RAM Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_IOST_MEM_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_iost_mem_mul_bit_ECC_err found! Memory Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_ITCT_MEM_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_itct_mem_mul_bit_ECC_err found! Memory Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_CQ_RAM_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_cq_ram_mul_bit_ECC_err found! RAM Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_NCQTAG_MEM0_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_ncq_tag_mem0_mul_bit_ECC_err found! Memory Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_NCQTAG_MEM1_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_ncq_tag_mem1_mul_bit_ECC_err found! Memory Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_NCQTAG_MEM2_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_ncq_tag_mem2_mul_bit_ECC_err found! Memory Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
	...
	if (irq_val & BIT(ECC_NCQTAG_MEM3_MULBIT_ERR_OFF)) {
		...
		dev_warn(dev, "%s card:%d Chip Error hgc_ncq_tag_mem3_mul_bit_ECC_err found! Memory Address is 0x%08X\n",
		    __func__, hisi_hba->core_id, reg_value);
		ret = -1;
	}
...
int ecc_err_check_v2_hw(struct hisi_hba *hisi_hba)
{
	...
	if (reg_val) {
		if (reg_val & BIT(AXI_ROB_MULBIT_ECC_ERR_OFF)) {
			...
			dev_warn(dev, "%s hba:%d Chip Error axi_rob_multib_ecc_err_cnt found ! cnt:%d\n",
			    __func__, hisi_hba->core_id,
		...
		if (irq_msk & (1 << phy_no)) {
			...
			if (irq_value1) {
				if (irq_value1 & (CHL_INT1_DMAC_RX_ECC_ERR_MSK |
					...
					dev_err(dev, "%s: DMAC RX/TX ecc bad error! (0x%x)",
			...
			if (irq_value2) {
				phy_err_code_process_v2_hw(hisi_hba,
					phy_no, irq_value2);
				hisi_sas_phy_write32(hisi_hba, phy_no,
						     CHL_INT2, irq_value2);
			}
		...
		if (hisi_hba->phy[i].port_id == port->id) {
			...
			if (reg_value == RCVR_HOLD_STATUS) {
				...
				hisi_sas_phy_write32(hisi_hba, i, TXID_AUTO,
							  txid_auto);
				dev_info(dev, "send break:phy_id:%d.\n", i);
			}
		}
	}
}
static const struct hisi_sas_hw hisi_sas_v2_hw = {
	...
	.chip_fatal_check = ecc_err_check_v2_hw,
