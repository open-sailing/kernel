...
static void hisi_sas_phyup_work(struct work_struct *work)
{
	...
	if (phy->is_flutter) {
		dev_info(dev, "%s,ignore flutter event of phy%d up!\n",
			__func__, phy_no);
		return;
	}
...
static void hisi_sas_refresh_devices_on_port(struct hisi_hba *hisi_hba,
...
static void hisi_sas_port_notify_formed(struct asd_sas_phy *sas_phy)
{
	...
	port = container_of(sas_port, typeof(*port), sas_port);

	/* preserve the old port id before hotplug or controller reset */
	old_port_id = port->id;
	...
	if ((old_port_id != HISI_SAS_INVALID_PORT_ID)
		...
		dev_info(dev, "port id has changed:phy(%d) new_port_id(%d) old_port_id(%d)\n",
			sas_phy->id, (u8)phy->port_id, old_port_id);

		/* the hardware port id of the sas port of
		 * the device is changed after hotplug, we shall
		 * refresh the ITCT entries
		 * of the devices that attached to this port
		 */
...
static void hisi_sas_port_deformed(struct asd_sas_phy *sas_phy)
{
	...
	if (port) {
		port->phy_bitmap &= ~BIT(sas_phy->id);
		if (!port->phy_bitmap) {
			dev_info(dev, "%s, sas port(%d) hardware port(%d) has deformed!\n",
				__func__, sas_port->id, port->id);
			port->port_attached = 0;
			port->id = HISI_SAS_INVALID_PORT_ID;
			sas_port->lldd_port = NULL;
		}
	}
	spin_unlock_irqrestore(&hisi_hba->lock, flags);
}
