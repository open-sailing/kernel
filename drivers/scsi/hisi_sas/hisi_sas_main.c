		...
		struct hisi_sas_io_context *context)
{
	...
	case SAS_PROTOCOL_SATA | SAS_PROTOCOL_STP:
		...
		break;
	default:
		pr_err("task prep: unknown/unsupported proto (0x%x)\n",
			task->task_proto);
		rc = -EINVAL;
		break;
	}
			      ...
			      int *pass)
{
	...
	if (unlikely(hisi_hba->rst_flag)) {
		dev_info(dev, "sas controller is resetting, reject io %p.\n",
				task);
		return -EINVAL;
	}
	...
	if (unlikely(dev_is_sata(device)
		...
		dev_info(dev, "sata disk(dev %p task %p) is recovering!\n",
			device, task);
	...
	if (!hisi_sas_slot_fsm(slot, &context)) {
		...
		dev_err(dev, "start io fail, task=%p\n", task);
...
#define HISI_SAS_SRST_ATA_DISK_COUNT (3)
static int hisi_sas_init_disk(struct domain_device *device)
{
	int rc = TMF_RESP_FUNC_FAILED;
	struct scsi_lun lun;
	struct hisi_sas_tmf_task tmf_task;
	int retry = HISI_SAS_SRST_ATA_DISK_COUNT;
				  ...
				  int task_tag)
{
	...
	if (unlikely(hisi_hba->rst_flag)) {
		dev_info(dev, "%s, sas controller is resetting, reject io %p.\n",
			__func__, task);
		return -EINVAL;
	}
...
exit:
	dev_info(dev, "internal task abort: dev %016llx task=%p resp: 0x%x sts 0x%x\n",
		SAS_ADDR(device->sas_addr),
		task,
...
static int hisi_sas_softreset_ata_disk(struct domain_device *device)
{
	...
	if (rc == TMF_RESP_FUNC_COMPLETE) {
		...
		rc = hisi_sas_exec_internal_tmf_task(device, NULL, 0, &tmf);
		if (rc != TMF_RESP_FUNC_COMPLETE) {
			pr_info("%s, ata disk %016llx de-reset failed, device=%p rc=%d\n",
				__func__, SAS_ADDR(device->sas_addr),
				device, rc);
		}
	} else {
		pr_info("%s, ata disk %016llx reset failed, device=%p rc=%d\n",
...
void hisi_sas_cnh_eh_all_slots(struct hisi_hba *hisi_hba)
{
	int dev_id;
	struct hisi_sas_device *sas_dev;
	struct domain_device *device;
...
void hisi_sas_cnh_release_all_slots(struct hisi_hba *hisi_hba)
{
	int dev_id;
	struct hisi_sas_device *sas_dev;
	struct domain_device *device;
...
static void hisi_sas_set_chip_reset_flag(struct hisi_hba *hisi_hba, int state)
{
	unsigned long flags;

	spin_lock_irqsave(&hisi_hba->lock, flags);
	hisi_hba->rst_flag = state;
	spin_unlock_irqrestore(&hisi_hba->lock, flags);
}
...
static int hisi_sas_chip_reset(struct hisi_hba *hisi_hba)
{
	...
	hisi_sas_set_chip_reset_flag(hisi_hba, 1);

	hisi_hba->rst_start_time = jiffies;

	rc = hisi_sas_exec_soft_reset(hisi_hba);
	if (rc != 0) {
		pr_info("%s, hisi_sas_exec_soft_reset failed! rc=%d\n",
			__func__, rc);
		goto err_out;
	}
	...
	if (!rc)
		pr_info("%s, sas controller reset successfully, timecost %dms.\n",
		 __func__,
