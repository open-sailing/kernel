...
/* base addr of mbigen clear register*/
#define MG_IRQ_CLEAR		0xa000

/* bit0 - bit63 is reserved for SPI irq*/
#define RESERVED_CLEAR_REG	0x8

#define MG_LPI_CLEAR_REG_BASE	(RESERVED_CLEAR_REG + MG_IRQ_CLEAR)
...
static inline void get_mbigen_clear_reg(irq_hw_number_t hwirq,
					u32 *mask, u32 *addr)
{
	unsigned int ofst, nid;


	nid = GET_MBIGEN_NID(hwirq);
	hwirq = HWIRQ_OFFSET(hwirq);
	hwirq += (nid - 1) * IRQ_PER_MBIGEN_NODE;

	ofst = hwirq / 32 * 4;
	*mask = 1 << (hwirq % 32);
	*addr = ofst + MG_LPI_CLEAR_REG_BASE;
}

/* for 1610 no need to clear the irq status */
static void mbigen_eoi_irq(struct irq_data *data)
{
	struct mbigen_node *mgn = data->chip_data;
	u32 mask, addr;

	get_mbigen_clear_reg(data->hwirq, &mask, &addr);
	writel_relaxed(mask, mgn->base + addr);
