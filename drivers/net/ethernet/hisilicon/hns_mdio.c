...
static int hns_mdio_probe(struct platform_device *pdev)
{
	...
	syscon_np = of_parse_phandle(np, "subctrl_vbase", 0);
	if (syscon_np) {
		mdio_dev->subctrl_vbase = syscon_node_to_regmap(syscon_np);
		if (IS_ERR(mdio_dev->subctrl_vbase)) {
			dev_warn(&pdev->dev, "no syscon hisilicon\n");
			mdio_dev->subctrl_vbase = NULL;
		} else {
			if (!memcmp(syscon_np->name,
				    "wrap_ctrl", strlen("wrap_ctrl")))
				mdio_dev->sc_reg++;
		}
	} else {
		dev_warn(&pdev->dev, "no subctrl_vbase\n");
		mdio_dev->subctrl_vbase = NULL;
	}
