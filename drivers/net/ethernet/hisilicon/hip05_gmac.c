...
#define HIP05_RCB_RING_OFFSET(x)	(0x10000 + 0x10000 * x->ring_num)
#define HIP05_PPE_COMMON_OFFSET		0x20000
...
#define HIP05_CFG_QID_MODE		0x00
#define HIP05_PPE_CFG_XGE_MODE		0x80
#define HIP05_PPE_CFG_TAG_GEN		0x90
...
#define HIP05_PPE_CF_QID_MODE		5	/* support 6Port/0VM/0TC/16TAG */
#define HIP05_PPE_TNL_TAG_MODE		1

enum hip05_chip_id {
	HIP05_CHIP_ID_P660 = 0,
	HIP05_CHIP_ID_HI1610,
};

#define HIP05_IRQ_ID_BASE_P660		0
#define HIP05_IRQ_ID_BASE_HI1610	25

#define IS_XGE_PORT(x)			(x < 6)
...
static void hip05_hw_init(struct net_device *dev)
{
	...
	if (priv->ppe_cfg_xge_mode && IS_XGE_PORT(priv->port_id)) {
		regmap_write(priv->ppe_map, HIP05_PPE_CFG_TAG_GEN,
			     HIP05_PPE_TNL_TAG_MODE << 16);
		regmap_write(priv->ppe_map,
			     HIP05_PPE_COMMON_OFFSET + HIP05_CFG_QID_MODE,
			     HIP05_PPE_CF_QID_MODE << 8 | priv->ring_num);
	}
...
static void hip05_port_enable(struct hip05_priv *priv)
{
	...
	regmap_write(priv->ge_map, 0x1e0, val);
}
	...
	for (i = rx_ring->next_to_clean; i != rx_ring->next_to_use;) {
		...
		if (unlikely(skb)) {
			printk_ratelimited(KERN_ERR
					   "inconsistent rx_skb for refill\n");
			break;
		}
...
static void hip05_reset_ppe(struct net_device *dev)
{
	...
	val = HIP05_PPE4_RESET_REQ | HIP05_PPE5_RESET_REQ |
	      HIP05_PPE6_RESET_REQ | HIP05_PPE7_RESET_REQ |
	      HIP05_RCB1_RESET_REQ | HIP05_RCB2_RESET_REQ;

	writel_relaxed(val, priv->gmac_reset_base + 0xa48);

	mdelay(10);
	val = HIP05_PPE4_RESET_DREQ | HIP05_PPE5_RESET_DREQ |
	      HIP05_PPE6_RESET_DREQ | HIP05_PPE7_RESET_DREQ |
	      HIP05_RCB1_RESET_DREQ | HIP05_RCB2_RESET_DREQ;

	writel_relaxed(val, priv->gmac_reset_base + 0xa4c);
	writel_relaxed(0x1, priv->gmac_reset_base + 0xa88);
	mdelay(10);
	writel_relaxed(0x1, priv->gmac_reset_base + 0xa8c);
}
...
static const struct of_device_id hip05_of_match[] = {
	{.compatible = "hisilicon, hip05-mac", .data = (void *)HIP05_CHIP_ID_P660},
	{.compatible = "hisilicon, hip05-hi1610", .data = (void *)HIP05_CHIP_ID_HI1610},
	{},
};
...
static int hip05_dev_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "ring-num", &ring_num);
	if (ret  < 0 || ring_num < 0 || ring_num > 128) {
		dev_warn(dev, "no ring-num, set to default 0\n");
		ring_num = 0;
	}

	priv->ring_num = ring_num;

	of_id = of_match_device(hip05_of_match, &pdev->dev);
	if (of_id) {
		priv->chip_id = (unsigned long)of_id->data;
	} else {
		dev_warn(dev, "no chip-id, set to default 0\n");
		priv->chip_id = 0;
	}

	ret = of_property_read_u32(node, "port-id", &port_id);
	if (ret  < 0 || port_id < 0 || port_id > 7) {
	...
	if (priv->chip_id == HIP05_CHIP_ID_HI1610 && IS_XGE_PORT(priv->port_id))
		irq_id_base = HIP05_IRQ_ID_BASE_HI1610 + priv->ring_num * 3;
	else
		irq_id_base = HIP05_IRQ_ID_BASE_P660;

	priv->rx_irq = platform_get_irq(pdev, irq_id_base);
	if (priv->rx_irq <= 0) {
	...
	priv->tx_irq = platform_get_irq(pdev, irq_id_base + 1);
	if (priv->tx_irq <= 0) {
	...
	if (!mac_addr || (mac_addr && !is_valid_ether_addr(mac_addr))) {
		regmap_read(priv->ge_map, GE_STATION_ADDR_HIGH_2, &val);
		mac[0] = (val >> 8) & 0xff;
		mac[1] = val & 0xff;

		regmap_read(priv->ge_map, GE_STATION_ADDR_LOW_2, &val);
		mac[2] = (val >> 24) & 0xff;
		mac[3] = (val >> 16) & 0xff;
		mac[4] = (val >> 8) & 0xff;
		mac[5] = val & 0xff;
	...
	priv->rcb_ring_base = priv->rcb_common_base + HIP05_RCB_RING_OFFSET(priv);
