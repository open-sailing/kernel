...
static int hns_ae_wait_flow_down(struct hnae_handle *handle)
{
	...
	if (!dsaf_dev)
		return -EINVAL;
	ret = hns_dsaf_wait_pkt_clean(dsaf_dev, handle->eport_id);
	if (ret)
		return ret;
				      ...
				      u32 *tx_usecs_high, u32 *rx_usecs_high)
{
	...
	*tx_frames_high = (dsaf_dev->desc_num - 1 > HNS_RCB_TX_FRAMES_HIGH) ?
...
static struct hnae_ae_ops hns_dsaf_ops = {
	...
	.set_uc_addr = hns_ae_set_unicast,
	.clr_uc_addr = hns_ae_clr_unicast,
	.add_rx_vlan = hns_ae_add_rx_vlan,
	.del_rx_vlan = hns_ae_del_rx_vlan,
