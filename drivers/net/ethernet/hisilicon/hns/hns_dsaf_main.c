	...
	if (!AE_IS_VER1(dsaf_dev->dsaf_ver)) {
		for (i = 0; i < DSAF_COMM_CHN; i++) {
			...
			snprintf(irq_info->irq_name,
				 EVENT_NAME_LEN, "xbar_xge_irq%d", i);
			irq_info->irq_name[EVENT_NAME_LEN - 1] = '\0';
		}
		...
		for (i = 0; i < DSAF_COMM_CHN; i++) {
			...
			snprintf(irq_info->irq_name,
				 EVENT_NAME_LEN, "xbar_ppe_irq%d", i);
			irq_info->irq_name[EVENT_NAME_LEN - 1] = '\0';
		}
	}
 ...
 * @sw_port: switch port index, 0~5 for MAC chanel, 6 for all PPE chanels, 7 for
 *	     all ROCEE chanels
 * @port_type: DSAF_SW_PORT_TYPE
 */
void hns_dsaf_sw_port_type_cfg(struct dsaf_device *dsaf_dev, u32 sw_port,
			       enum dsaf_sw_port_type port_type)
{
	dsaf_set_dev_field(dsaf_dev,
			   DSAF_SW_PORT_TYPE_0_REG + 0x0004 * sw_port,
...
/**
 * hns_dsaf_sw_port_type_rstr - restore sw port type based on vlan dev number
 *				when reset port
 * @dsaf_dev: dsaf device
 * @port: service port mac id, 0~5
 */
...
/* this param is used for sbm in (not pfc)/2TC/4TC */
static const u32 sbm_bp_wl_cfg_xge_cfg[SBM_BP_CFG_MODE_MAX][11] = {
	{256, 0, 0, 0, 0, 110, 140, 55, 110, 128, 192},
	{32, 80, 80, 32, 32, 110, 140, 55, 110, 55, 110},
	{32, 112, 112, 0, 0, 110, 140, 55, 110, 55, 110},
};

/* Used for when enable or disable pfc, the wl changed. */
				...
				int channel, u8 pfc_en)
{
	...
	dsaf_set_dev_bit(dsaf_dev, DSAF_SBM_CFG_REG_0_REG + 0x80 * channel,
			 DSAF_SBM_CFG_MIB_EN_S, 0);

	dsaf_write_dev(dsaf_dev, DSAF_PFC_EN_0_REG + 4 * channel, pfc_en);

	cnt = 0;
	/* now only support 2TC */
	for (i = 0; i < DSAF_TC_CHANNEL_NUM; i++) {
		if (pfc_en & (1 << i))
			cnt++;
	}
	...
	switch (cnt) {
	case 0:
		cfg_idx = SBM_DIASBLE_ALL_TC;
		break;
	case 1:
	case 2:
		cfg_idx = SBM_ENABLE_2TC_MODE;
		break;
	default:
		cfg_idx = SBM_ENABLE_4TC_MODE;
		break;
	}

	reg = DSAF_SBM_BP_CFG_4_REG_0_REG + 0x80 * channel;
	o_sbm_bp_cfg4 = dsaf_read_dev(dsaf_dev, reg);
	dsaf_set_field(o_sbm_bp_cfg4,
		       DSAFV2_SBM_CFG4_SET_BUF_NUM_NO_PFC_M,
		       DSAFV2_SBM_CFG4_SET_BUF_NUM_NO_PFC_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][9]);
	dsaf_set_field(o_sbm_bp_cfg4,
		       DSAFV2_SBM_CFG4_RESET_BUF_NUM_NO_PFC_M,
		       DSAFV2_SBM_CFG4_RESET_BUF_NUM_NO_PFC_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][10]);
	dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg4);

	reg = DSAF_SBM_BP_CFG_3_REG_0_REG + 0x80 * channel;
	o_sbm_bp_cfg3 = dsaf_read_dev(dsaf_dev, reg);
	dsaf_set_field(o_sbm_bp_cfg3,
		       DSAFV2_SBM_CFG3_SET_BUF_NUM_NO_PFC_M,
		       DSAFV2_SBM_CFG3_SET_BUF_NUM_NO_PFC_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][7]);
	dsaf_set_field(o_sbm_bp_cfg3,
		       DSAFV2_SBM_CFG3_RESET_BUF_NUM_NO_PFC_M,
		       DSAFV2_SBM_CFG3_RESET_BUF_NUM_NO_PFC_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][8]);
	dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg3);

	reg = DSAF_SBM_BP_CFG_2_XGE_REG_0_REG + 0x80 * channel;
	o_sbm_bp_cfg2 = dsaf_read_dev(dsaf_dev, reg);
	dsaf_set_field(o_sbm_bp_cfg2, DSAFV2_SBM_CFG2_SET_BUF_NUM_M,
		       DSAFV2_SBM_CFG2_SET_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][5]);
	dsaf_set_field(o_sbm_bp_cfg2, DSAFV2_SBM_CFG2_RESET_BUF_NUM_M,
		       DSAFV2_SBM_CFG2_RESET_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][6]);
	dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg2);

	reg = DSAF_SBM_BP_CFG_0_XGE_REG_0_REG + 0x80 * channel;
	o_sbm_bp_cfg0 = dsaf_read_dev(dsaf_dev, reg);
	dsaf_set_field(o_sbm_bp_cfg0, DSAFV2_SBM_CFG0_COM_MAX_BUF_NUM_M,
		       DSAFV2_SBM_CFG0_COM_MAX_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][0]);
	dsaf_set_field(o_sbm_bp_cfg0, DSAFV2_SBM_CFG0_VC0_MAX_BUF_NUM_M,
		       DSAFV2_SBM_CFG0_VC0_MAX_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][1]);
	dsaf_set_field(o_sbm_bp_cfg0, DSAFV2_SBM_CFG0_VC1_MAX_BUF_NUM_M,
		       DSAFV2_SBM_CFG0_VC1_MAX_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][2]);
	dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg0);

	reg = DSAF_SBM_BP_CFG_1_REG_0_REG + 0x80 * channel;
	o_sbm_bp_cfg1 = dsaf_read_dev(dsaf_dev, reg);
	dsaf_set_field(o_sbm_bp_cfg1, DSAFV2_SBM_CFG1_TC4_MAX_BUF_NUM_M,
		       DSAFV2_SBM_CFG1_TC4_MAX_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][3]);
	dsaf_set_field(o_sbm_bp_cfg1, DSAFV2_SBM_CFG1_TC0_MAX_BUF_NUM_M,
		       DSAFV2_SBM_CFG1_TC0_MAX_BUF_NUM_S,
		       sbm_bp_wl_cfg_xge_cfg[cfg_idx][4]);
	dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg1);

	dsaf_set_dev_bit(dsaf_dev, DSAF_SBM_CFG_REG_0_REG + 0x80 * channel,
			 DSAF_SBM_CFG_MIB_EN_S, 1);
...
static void hns_dsafv2_sbm_bp_wl_cfg(struct dsaf_device *dsaf_dev)
{
	...
	for (i = 0; i < DSAFV2_SBM_XGE_CHN; i++) {
		...
		dsaf_set_field(o_sbm_bp_cfg, DSAFV2_SBM_CFG2_SET_BUF_NUM_M,
			       DSAFV2_SBM_CFG2_SET_BUF_NUM_S, 120);
		dsaf_set_field(o_sbm_bp_cfg, DSAFV2_SBM_CFG2_RESET_BUF_NUM_M,
			       DSAFV2_SBM_CFG2_RESET_BUF_NUM_S, 170);
		dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg);
	...
	for (i = 0; i < DSAFV2_SBM_PPE_CHN; i++) {
		reg = DSAF_SBM_BP_CFG_2_PPE_REG_0_REG + 0x80 * i;
		o_sbm_bp_cfg = dsaf_read_dev(dsaf_dev, reg);
		dsaf_set_field(o_sbm_bp_cfg, DSAF_SBM_CFG2_PPE_SET_BUF_NUM_M,
			       DSAF_SBM_CFG2_PPE_SET_BUF_NUM_S, 2);
		dsaf_set_field(o_sbm_bp_cfg, DSAF_SBM_CFG2_PPE_RESET_BUF_NUM_M,
			       DSAF_SBM_CFG2_PPE_RESET_BUF_NUM_S, 3);
		dsaf_set_field(o_sbm_bp_cfg, DSAF_SBM_CFG2_PPE_CFG_USEFUL_NUM_M,
			       DSAF_SBM_CFG2_PPE_CFG_USEFUL_NUM_S, 52);
		dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg);
	}
	...
	for (i = 0; i < DASFV2_ROCEE_CRD_NUM; i++) {
		...
		o_sbm_bp_cfg = dsaf_read_dev(dsaf_dev, reg);
		dsaf_set_field(o_sbm_bp_cfg, DSAF_SBM_CFG2_ROCEE_SET_BUF_NUM_M,
			       DSAF_SBM_CFG2_ROCEE_SET_BUF_NUM_S, 2);
		dsaf_set_field(o_sbm_bp_cfg,
			       DSAF_SBM_CFG2_ROCEE_RESET_BUF_NUM_M,
			       DSAF_SBM_CFG2_ROCEE_RESET_BUF_NUM_S, 4);
		dsaf_write_dev(dsaf_dev, reg, o_sbm_bp_cfg);
	}
}
...
/**
 * hns_dsaf_tbl_tcam_match_cfg - tbl
 * @dsaf_dev: dsa fabric device struct pointer
 * @ptbl_tcam_data: addr
 */
static inline void hns_dsaf_tbl_tcam_match_cfg(
	struct dsaf_device *dsaf_dev,
 ...
 * @dsaf_dev: dsa fabric device struct pointer
 * @address: tcam table address
 * @ptbl_tcam_data: tcam data
 * @ptbl_tcam_ucast: unicast configuration
 */
 ...
 * @ptbl_tcam_data,
 */
static void hns_dsaf_tcam_uc_cfg_vague(
	struct dsaf_device *dsaf_dev, u32 address,
	struct dsaf_tbl_tcam_data *ptbl_tcam_data,
 ...
 * @ptbl_tcam_mcast
 */
static void hns_dsaf_tcam_mc_cfg_vague(
	struct dsaf_device *dsaf_dev, u32 address,
	struct dsaf_tbl_tcam_data *ptbl_tcam_data,
	struct dsaf_tbl_tcam_data *ptbl_tcam_mask,
	struct dsaf_tbl_tcam_mcast_cfg *ptbl_tcam_mcast)
...
/**
 * hns_dsaf_tcam_addr_get - INT
 * @mac_key: tcam mac_key
 * @addr: mac addr output
 */
...
void hns_dsaf_pfc_set_pause_en(struct dsaf_device *dsaf_dev, int mac_id,
			       u8 en)
{
	dsaf_set_dev_bit(dsaf_dev, DSAF_PAUSE_CFG_REG + mac_id * 4, 0, !!en);
	dsaf_set_dev_bit(dsaf_dev, DSAF_PAUSE_CFG_REG + mac_id * 4, 1, !!en);
}
			       ...
			       u8 *en)
{
	...
	pfc_en = dsaf_get_dev_field(dsaf_dev, DSAF_PAUSE_CFG_REG + mac_id * 4,
				    0x3, 0);
	if (pfc_en != 0x3 && pfc_en != 0)
		dev_err(dsaf_dev->dev, "PFC is not all enable or all disable, value = %d.",
			pfc_en);
...
void hns_dsaf_pfc_set_tc_en(struct dsaf_device *dsaf_dev, int mac_id,
			    u8 tc, u8 en)
{
	dsaf_set_dev_bit(dsaf_dev, DSAF_PFC_EN_0_REG + mac_id * 4, tc, !!en);
}
			    ...
			    u8 tc, u8 *en)
{
	*en = dsaf_get_dev_bit(dsaf_dev,
			       DSAF_PFC_EN_0_REG + mac_id * 4, tc);
}

void hns_dsaf_ets_get_up2tc(struct dsaf_device *dsaf_dev, int mac_id,
			    u32 *prio_tc)
{
	*prio_tc = dsaf_read_dev(dsaf_dev,
				 DSAF_INODE_PRI_TC_CFG_0_REG + 0x80 * mac_id);
}

void hns_dsaf_ets_set_up2tc(struct dsaf_device *dsaf_dev, int mac_id,
			    u32 prio_tc)
{
	/* no vlan tag data and man frame can not be configured. use default. */
	dsaf_set_field(prio_tc, DSAF_INODE_NO_VLAN_DATA_TC_M,
		       DSAF_INODE_NO_VLAN_DATA_TC_S,
		       DEFAULT_INODE_NO_VLAN_DATA_TC);

	dsaf_set_field(prio_tc, DSAF_INODE_NO_VLAN_MAN_TC_M,
		       DSAF_INODE_NO_VLAN_MAN_TC_S,
		       DEFAULT_INODE_NO_VLAN_MAN_TC);

	dsaf_write_dev(dsaf_dev,
		       DSAF_INODE_PRI_TC_CFG_0_REG + 0x80 * mac_id, prio_tc);
}
...
static void hns_dsaf_comm_init(struct dsaf_device *dsaf_dev)
{
	...
	if (!is_ver1)
		dsaf_set_bit(o_dsaf_cfg, DSAFV2_CFG_VLAN_TAG_MODE_S, 0);
	...
	/* set all mac ports to non vlan mode */
	for (i = 0; i < DSAF_SW_PORT_MAC_NUM; i++)
		hns_dsaf_sw_port_type_cfg(dsaf_dev, i,
					  DSAF_SW_PORT_TYPE_NON_VLAN);
	/* set ppe port to non vlan mode */
	hns_dsaf_sw_port_type_cfg(dsaf_dev, DSAF_SW_PORT_PPE_IDX,
				  DSAF_SW_PORT_TYPE_NON_VLAN);
	/* set rocee port to access mode */
	hns_dsaf_sw_port_type_cfg(dsaf_dev, DSAF_SW_PORT_ROCEE_IDX,
				  DSAF_SW_PORT_TYPE_ACCESS);
...
static void hns_dsaf_inode_init(struct dsaf_device *dsaf_dev)
{
	...
	for (i = 0; i < DSAF_INODE_NUM; i++) {
		...
		dsaf_write_dev(dsaf_dev, reg, HNS_DSAF_I8TC_CFG);
	}
}
...
/**
 * hns_dsaf_check_init_fini - INT
 * @dsaf_dev: dsa device
 */
static int hns_dsaf_check_init_fini(struct dsaf_device *dsaf_dev)
{
	u32 flag;
	u32 finish_msk;
	u32 cnt = 0;
	...
	do {
		...
		cnt++;
	} while (flag != finish_msk && cnt < DSAF_CFG_READ_CNT);

	if (flag != finish_msk) {
		dev_err(dsaf_dev->dev,
			"hns_dsaf_check_init_fini fail %s, flag=%d, cnt=%d\n",
...
int hns_dsaf_init_hw(struct dsaf_device *dsaf_dev)
{
	...
	/*enable all clock*/
	hns_dsaf_clk_enable_all(dsaf_dev);

	/*init VOQ*/
	hns_dsaf_voq_init(dsaf_dev);

	/*check init finish*/
	ret = hns_dsaf_check_init_fini(dsaf_dev);
	if (ret)
		return ret;
	...
	struct dsaf_drv_mac_single_dest_entry *mask_entry)
{
	...
	if (mask_entry) {
		...
		hns_dsaf_set_mac_key(dsaf_dev, &mask_key,
				     mask_entry->in_vlan_id,
				     mask_entry->in_port_num,
				     mask_entry->addr);
		/* config unicast entry with mask */
		hns_dsaf_tcam_uc_cfg_vague(dsaf_dev, entry_index, &tcam_data,
					   (struct dsaf_tbl_tcam_data *)
					   (&mask_key), &mac_data);
	} else {
		hns_dsaf_tcam_uc_cfg(dsaf_dev, entry_index,
				     &tcam_data, &mac_data);
	}
...
/**
 * hns_dsaf_add_mac_mc_port_vague - add mac mc-port
 * @dsaf_dev: dsa fabric device struct pointer
 * @mac_entry: mc-mac entry
 */
int hns_dsaf_add_mac_mc_port_vague(
	struct dsaf_device *dsaf_dev,
	struct dsaf_drv_mac_single_dest_entry *mac_entry,
	struct dsaf_drv_mac_single_dest_entry *mask_entry)
{
	...
	if (mask_entry) {
		...
		hns_dsaf_tcam_mc_cfg_vague(dsaf_dev, entry_index, &tcam_data,
					   (struct dsaf_tbl_tcam_data *)
					   (&mask_key), &mac_data);
	} else {
 ...
 * @mac_entry: mac entry
 */
int hns_dsaf_del_mac_mc_port_vague(
	struct dsaf_device *dsaf_dev,
	struct dsaf_drv_mac_single_dest_entry *mac_entry,
	struct dsaf_drv_mac_single_dest_entry *mask_entry)
{
	...
	if (!memcmp(mac_data.tbl_mcast_port_msk, empty_msk,
		...
		if (mask_entry) {
			/*config key mask */
			hns_dsaf_set_mac_key(dsaf_dev,
					     &mask_key, mask_entry->in_vlan_id,
					     mask_entry->in_port_num,
					     mask_entry->addr);
			     ...
			     struct dsaf_drv_mac_single_dest_entry *mac_entry)
{
	...
	/* mask ipv6 multicast mac addr */
	if ((mac_entry->addr[0] == 0x33) && (mac_entry->addr[1] == 0x33)) {
		memset(&entry_tmp.addr[2], 0x00, MAC_NUM_OCTETS_PER_ADDR - 2);
		memset(&entry_mask.addr[2], 0x00, MAC_NUM_OCTETS_PER_ADDR - 2);
	}

	return hns_dsaf_add_mac_mc_port_vague(dsaf_dev, &entry_tmp,
					      &entry_mask);
}
			     ...
			     struct dsaf_drv_mac_single_dest_entry *mac_entry)
{
	...
	return hns_dsaf_del_mac_mc_port_vague(dsaf_dev, &entry_tmp,
					      &entry_mask);
}

int hns_dsaf_enable_service_promisc(struct dsaf_device *dsaf_dev, u32 port)
{
	u16 entry_index = DSAF_INVALID_ENTRY_IDX;
	u8 port_num;
	struct hns_mac_cb *mac_cb;
	struct dsaf_drv_tbl_tcam_key mac_key;
	struct dsaf_tbl_tcam_data tbl_tcam_data = {0, port};
	struct dsaf_tbl_tcam_data tbl_tcam_mask = {0, 0xf};
	...
	if (entry_index == DSAF_INVALID_ENTRY_IDX) {
		dev_err(dsaf_dev->dev,
			"enable promisc mode failed (port num:%#x)\n",	port);
		return -EINVAL;
	}
	...
	hns_dsaf_tcam_uc_cfg_vague(dsaf_dev, entry_index, &tbl_tcam_data,
...
int hns_dsaf_disable_service_promisc(struct dsaf_device *dsaf_dev, u32 port)
{
	u16 entry_index = DSAF_INVALID_ENTRY_IDX;
	struct dsaf_drv_tbl_tcam_key mac_key;
...
int hns_dsaf_enable_debug_promisc(struct dsaf_device *dsaf_dev, u32 port)
{
	struct hns_mac_cb *mac_cb = &dsaf_dev->mac_cb[port];

	return hns_mac_set_promisc(mac_cb, 1);
}

int hns_dsaf_disable_debug_promisc(struct dsaf_device *dsaf_dev, u32 port)
{
	struct hns_mac_cb *mac_cb = &dsaf_dev->mac_cb[port];

	return hns_mac_set_promisc(mac_cb, 0);
}

/**
 * hns_dsaf_set_promisc - set promisc mode
 * @dsaf_dev: dsa fabric device struct pointer
 * @port: port number
 * @enable: enable or disable promisc mode
 */
void hns_dsaf_set_promisc(struct dsaf_device *dsaf_dev, u32 port, u32 enable)
{
	if (enable == 0) {
		/* disable nic promisc mode */
		if (port < DSAF_SERVICE_NW_NUM)
			(void)hns_dsaf_disable_service_promisc(dsaf_dev, port);
		else
			(void)hns_dsaf_disable_debug_promisc(dsaf_dev, port);
	} else {
		/* enable nic promisc mode */
		if (port < DSAF_SERVICE_NW_NUM)
			(void)hns_dsaf_enable_service_promisc(dsaf_dev, port);
		else
			(void)hns_dsaf_enable_debug_promisc(dsaf_dev, port);
	}
}
...
static irqreturn_t xbar_xge_int_handler(int irq, void *dev)
{
	...
	if (dsaf_get_bit(int_sts, DSAF_SBM_XGE_SRAM_ECC_2BIT_INT))
		net_edac_dev_err(dsaf_dev->dev, port_id,
	...
	if (flag != HNS_OK_STATE) {
		dsaf_write_dev(dsaf_dev, DSAF_XGE_INT_MSK_0_REG +
			       (port_id << REG_STEP_SHIFT),
			       HNS_DSAF_IRQ_DISABLE_MSK);
		if (dsaf_dev->ae_dev.callback_ops->event_report)
			dsaf_dev->ae_dev.callback_ops->event_report(
				0, port_id, NIC_DEVT_PORT_CHIP_RECOVER,
				NIC_DEVICE_PORT, flag);
		return IRQ_HANDLED;
	}
...
static irqreturn_t xbar_ppe_int_handler(int irq, void *dev)
{
	...
	if (flag != HNS_OK_STATE) {
		...
		if (dsaf_dev->ae_dev.callback_ops->event_report)
			dsaf_dev->ae_dev.callback_ops->event_report(
				0, port_id, flag == HNS_DSAF_RSTING_STATE ?
				NIC_DEVT_DSAF_CHIP_RECOVER :
				NIC_DEVT_PORT_CHIP_RECOVER,
				NIC_DEVICE_PORT, flag);
		return IRQ_HANDLED;
	}
...
int hns_dsaf_xbar_irq_init(struct dsaf_device *dsaf_dev, u32 port_id)
{
	...
	hns_dsaf_irq_set(dsaf_dev, port_id, 1);
...
void hns_dsaf_waterline_ex_init(struct dsaf_device *dsaf_dev)
{
	...
	for (i = 0; i < DSAFV2_SBM_XGE_CHN; i++) {
		...
		/* Current we don't change GE's water line settings. */
...
void hns_dsaf_irq_set(struct dsaf_device *dsaf_dev, int port, int en)
{
	dsaf_write_dev(dsaf_dev,
		       ...
		       en ? HNS_DSAF_XGE_IRQ_ENABLE_MSK :
		       HNS_DSAF_IRQ_DISABLE_MSK);
	dsaf_write_dev(dsaf_dev,
		       (DSAF_PPE_INT_MSK_0_REG + (port << REG_STEP_SHIFT)),
		       en ? HNS_DSAF_PPE_IRQ_ENABLE_MSK :
		       HNS_DSAF_IRQ_DISABLE_MSK);
}
 ...
 * retuen 0 - success , negative --fail
 */
int hns_dsaf_roce_reset(struct fwnode_handle *dsaf_fwnode, u32 port_mode,
			u32 val)
{
	struct dsaf_device *dsaf_dev;
	struct platform_device *pdev;
	...
	pdev = of_find_device_by_node(of_node(dsaf_fwnode));
	...
	if (!val) {
		...
		for (i = 0; i < DSAF_ROCE_CREDIT_CHN; i++)
			dsaf_set_field(mp, 7 << i * 3, i * 3,
				       port_map[i][port_mode]);
		dsaf_set_field(mp, 3 << i * 3, i * 3, port_mode);
