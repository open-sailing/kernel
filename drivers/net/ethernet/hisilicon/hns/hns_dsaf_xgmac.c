...
/**
 *hns_xgmac_set_tx_lf_rf_control - set tx lf rf control about xgmac
 *@mac_drv: mac driver
 *@enable:enable rx pause param
 */
static void hns_xgmac_set_tx_lf_rf_control(void *mac_drv, u32 val)
{
	struct mac_driver *drv = (struct mac_driver *)mac_drv;

	dsaf_write_dev(drv, XGMAC_MAC_TX_LF_RF_CONTROL_REG, val);
}
...
static void hns_xgmac_enable(void *mac_drv, enum mac_commom_mode mode)
{
	...
	hns_xgmac_set_tx_lf_rf_control(drv, XGE_TX_LF_RF_ENABLE);
...
static void hns_xgmac_disable(void *mac_drv, enum mac_commom_mode mode)
{
	...
	hns_xgmac_set_tx_lf_rf_control(drv, XGE_TX_LF_RF_DISABLE);
}
...
static void hns_xgmac_init(void *mac_drv)
{
	...
	hns_xgmac_config_pad_and_crc(mac_drv, 1);
}
...
{
	dsaf_write_dev(mac_drv, XGMAC_INT_ENABLE_REG, XGE_INT_DISABLE);
}
...
static int hns_xgmac_get_irq(struct platform_device *pdev, void *mac_drv)
{
	...
	snprintf(drv->mac_cb->irq_info.irq_name, EVENT_NAME_LEN,
		 "xge_event_irq%d", drv->mac_id);
	drv->mac_cb->irq_info.irq_name[EVENT_NAME_LEN - 1] = '\0';
