...
#define PHY_LOOP_BACK BIT(14)
	...
	if (en) {
		/* speed : 1000M */
		(void)hns_nic_set_mdio_reg(net_dev, 2, 21, 0x1046);

		/* Force Master */
		(void)hns_nic_set_mdio_reg(net_dev, 0, 9, 0x1F00);

		/* Soft-reset */
		(void)hns_nic_set_mdio_reg(net_dev, 0, 0, 0x9140);

		/* If autoneg disabled,two soft-reset operations */
		(void)hns_nic_set_mdio_reg(net_dev, 0, 0, 0x9140);

		/* Default is 0x0400 */
		(void)hns_nic_set_mdio_reg(net_dev, 0xFA, 1, 0x418);

		/* Force 1000M Link, Default is 0x0200 */
		(void)hns_nic_set_mdio_reg(net_dev, 0xFA, 7, 0x20C);

		/* Enable PHY loop-back */
		(void)hns_nic_get_mdio_reg(net_dev, 0, COPPER_CONTROL_REG,
					   &val);
		val |= PHY_LOOP_BACK;
		val &= ~PHY_POWER_DOWN;
		...
		(void)hns_nic_set_mdio_reg(net_dev, 0xFA, 1, 0x400);
		(void)hns_nic_set_mdio_reg(net_dev, 0xFA, 7, 0x200);
