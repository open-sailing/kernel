...
static u32 hns_nic_get_link(struct net_device *net_dev)
{
	...
	return hns_nic_get_status(net_dev, priv->link);
}

static void hns_get_mdix_mode(struct net_device *net_dev,
			      struct ethtool_cmd *cmd)
{
	int mdix_ctrl, mdix, is_resolved, ret;
	u32 regval;
	struct hns_nic_priv *priv = netdev_priv(net_dev);
	...
	ret = hns_nic_get_mdio_reg(net_dev, HNS_PHY_PAGE_MDIX,
				   HNS_PHY_CSC_REG, &regval);
	if (ret < 0) {
		netdev_err(net_dev, "hns_nic_get_mdio_reg fail ret %d\n", ret);
		return;
	}

	mdix_ctrl = hnae_get_field(regval, PHY_MDIX_CTRL_M, PHY_MDIX_CTRL_S);

	ret = hns_nic_get_mdio_reg(net_dev, HNS_PHY_PAGE_MDIX,
				   HNS_PHY_CSS_REG, &regval);
	if (ret < 0) {
		netdev_err(net_dev, "hns_nic_get_mdio_reg fail ret %d\n", ret);
		return;
	}

	mdix = hnae_get_bit(regval, PHY_MDIX_STATUS_B);
	is_resolved = hnae_get_bit(regval, PHY_SPEED_DUP_RESOLVE_B);
...
static int hns_nic_config_phy_loopback(struct net_device *net_dev, u8 en)
{
...
#define PHY_LOOP_BACK BIT(14)
	...
	if (en) {
		/* speed : 1000M */
		hns_nic_set_mdio_reg(net_dev, 2, 21, 0x1046);

		/* Force Master */
		hns_nic_set_mdio_reg(net_dev, 0, 9, 0x1F00);

		/* Soft-reset */
		hns_nic_set_mdio_reg(net_dev, 0, 0, 0x8140);

		/* If autoneg disabled,two soft-reset operations */
		hns_nic_set_mdio_reg(net_dev, 0, 0, 0x8140);

		/* Default is 0x0400 */
		hns_nic_set_mdio_reg(net_dev, 0xFA, 1, 0x418);

		/* Force 1000M Link, Default is 0x0200 */
		hns_nic_set_mdio_reg(net_dev, 0xFA, 7, 0x20C);

		/* Enable PHY loop-back */
		hns_nic_get_mdio_reg(net_dev, 0, COPPER_CONTROL_REG, &val);
		val |= PHY_LOOP_BACK;
		val &= ~PHY_POWER_DOWN;
		hns_nic_set_mdio_reg(net_dev, 0, COPPER_CONTROL_REG, val);
		msleep(1000);
	} else {
		/* dont Force Master */
		hns_nic_set_mdio_reg(net_dev, 0, 9, 0x0F00);
		hns_nic_set_mdio_reg(net_dev, 0xFA, 1, 0x400);
		hns_nic_set_mdio_reg(net_dev, 0xFA, 7, 0x200);

		hns_nic_get_mdio_reg(net_dev, 0, COPPER_CONTROL_REG, &val);
		val &= ~PHY_LOOP_BACK;
		val |= PHY_POWER_DOWN;
		(void)hns_nic_set_mdio_reg(net_dev, 0, COPPER_CONTROL_REG, val);
	}
	return 0;
}
		      ...
		      enum hnae_loop loop)
{
	...
	case MAC_INTERNALLOOP_PHY:
		if ((phy_dev) && (!phy_dev->is_c45)) {
			ret = hns_nic_config_phy_loopback(ndev, 0x1);
	...
	case MAC_LOOP_NONE:
		if ((phy_dev) && (!phy_dev->is_c45))
			ret |= hns_nic_config_phy_loopback(ndev, 0x0);
		   ...
		   enum hnae_loop loop_mode)
{
	...
	if (loop_mode == MAC_INTERNALLOOP_PHY && priv->phy) {
...
#define NIC_LB_TEST_RX_PKG_ERR 4
	...
	for (j = 0; j < lc; j++) {
		...
		for (i = 0; i < NIC_LB_TEST_PKT_NUM_PER_CYCLE; i++) {
			...
			tx_ret_val = hns_nic_net_xmit_hw(
				ndev, skb,
...
static int __lb_down(struct net_device *ndev)
{
	...
	__lb_clean_rings(priv, 0, h->q_num - 1, 256);
			       ...
			       struct ethtool_pauseparam *param)
{
	...
	param->autoneg = priv->link_config.pause_autoneg;
	if (priv->link_config.pause_autoneg == AUTONEG_ENABLE) {
		param->rx_pause = !!(priv->link_config.flowctrl & FLOW_CTRL_RX);
		param->tx_pause = !!(priv->link_config.flowctrl & FLOW_CTRL_TX);
	} else {
		ops->get_pauseparam(priv->ae_handle, NULL,
			      ...
			      struct ethtool_pauseparam *param)
{
	...
	phydev = priv->phy;
	if (phydev) {
		memcpy(&priv->epause, param, sizeof(struct ethtool_pauseparam));
		spin_lock_bh(&priv->lock);
		priv->link_config.pause_autoneg = param->autoneg;
		spin_unlock_bh(&priv->lock);
		priv->link_config.flowctrl = 0;
		if (param->rx_pause) {
			priv->link_config.flowctrl |= FLOW_CTRL_RX;
			if (param->tx_pause) {
				/* rx pause & tx pause enable. */
				priv->link_config.flowctrl |= FLOW_CTRL_TX;
				newadv = ADVERTISED_Pause;
			} else {
				/* rx pause enable & tx pause disable. */
				newadv = ADVERTISED_Pause |
					 ADVERTISED_Asym_Pause;
			}
		} else if (param->tx_pause) {
			/* rx pause disable & tx pause enable */
			priv->link_config.flowctrl |= FLOW_CTRL_TX;
		...
		if (oldadv != newadv) {
			...
			/* Advertise pause settings to partner. */
			(void)phy_start_aneg(phydev);
		}

		/* If we DON'T allow autoneg,
		 * SET the wanted pause parameters to HW.
		 */
		if (phydev->autoneg == AUTONEG_ENABLE &&
		    param->autoneg == AUTONEG_ENABLE)
			return 0;
	}
	ret = ops->set_pauseparam(priv->ae_handle, param->autoneg,
				   param->rx_pause, param->tx_pause);
	if (ret)
		return ret;
...
int hns_phy_led_set(struct net_device *netdev, int value)
{
	...
	/* led0 is used for location led */
	retval = hns_nic_get_mdio_reg(netdev, HNS_PHY_PAGE_LED, HNS_LED_FC_REG,
				      &tempval);
	if (retval) {
		netdev_err(netdev, "hns_nic_get_mdio_reg fail!\n");
		return retval;
	}

	tempval = (tempval & 0xfff0) | value;
	retval = hns_nic_set_mdio_reg(netdev, HNS_PHY_PAGE_LED, HNS_LED_FC_REG,
				      tempval);
	if (retval) {
		netdev_err(netdev, "hns_nic_set_mdio_reg fail!\n");
		return retval;
	}
	return 0;
}
...
int hns_set_phys_id(struct net_device *netdev, enum ethtool_phys_id_state state)
{
	...
	if (phy_dev)
		...
		case ETHTOOL_ID_ACTIVE:
			ret = hns_nic_get_mdio_reg(netdev, HNS_PHY_PAGE_LED,
						   HNS_LED_FC_REG, &regval);
			if (ret)
				return ret;
			priv->phy_led_val = (int)regval;
			return 2;
		case ETHTOOL_ID_ON:
		...
		case ETHTOOL_ID_INACTIVE:
			ret =  hns_nic_set_mdio_reg(netdev, HNS_PHY_PAGE_LED,
						    HNS_LED_FC_REG,
						    (u32)priv->phy_led_val);
			if (ret)
				return ret;
			break;
		default:
...
static int hns_nic_nway_reset(struct net_device *netdev)
{
	...
	if (netif_running(netdev)) {
		/* if autoneg is disabled, it's an error to restart */
