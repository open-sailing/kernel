...
{
	struct ethhdr *hns_eth_hdr = (struct ethhdr *)skb->data;

	/* check if pkt is unicast */
	if (likely(!(hns_eth_hdr->h_dest[0] & 0x01)))
		return 0;

	/* check if smac_addr equals to local_mac_addr */
	if (memcmp(ndev_addr, hns_eth_hdr->h_source, ETH_ALEN))
		return 0;
	else
		return 1;
}
...
static void hns_nic_adjust_link(struct net_device *ndev)
{
	...
	if (phydev->link && phydev->duplex == DUPLEX_FULL) {
		lcl_adv = mii_advertise_flowctrl(priv->link_config.flowctrl);
		...
		newflowctrl = mii_resolve_flowctrl_fdx(lcl_adv, rmt_adv);
