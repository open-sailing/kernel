...
static const struct {
	...
	{HNAE_MEDIA_TYPE_UNKNWON,	"unknown" },
			   ...
			   u32 vmid, char *addr)
{
	...
	if (dsaf_dev) {
		...
		ret = hns_dsaf_set_mac_uc_entry(dsaf_dev, &entry_tmp,
						&entry_mask);
		if (ret)
			return ret;
	}
...
void hns_mac_reset(struct hns_mac_cb *mac_cb)
{
	...
	if (drv->config_max_frame_length)
		drv->config_max_frame_length(drv, is_ver1 ? MAC_MAX_MTU :
					     MAC_MAX_MTU_V2);
...
int hns_mac_add_rx_vlan(struct hns_mac_cb *mac_cb, u16 vlan_id)
{
	/* record vlan dev number */
	if (vlan_id != 0)
		mac_cb->vids++;
	return 0;
}

int hns_mac_del_rx_vlan(struct hns_mac_cb *mac_cb, u16 vlan_id)
{
	/* record vlan dev number */
	if (vlan_id != 0)
		mac_cb->vids--;
...
void hns_mac_init_hw(struct hns_mac_cb *mac_cb)
{
	struct mac_driver *mac_drv = hns_mac_get_drv(mac_cb);

	if (mac_cb->dsaf_dev->cpld_base &&
	    mac_cb->mac_id < DSAF_SERVICE_PORT_NUM_PER_DSAF)
		cpld_led_reset(mac_cb);
	...
	/* set mac's max_pkt_size to max value */
	if (mac_drv->config_max_frame_length)
		mac_drv->config_max_frame_length(
				mac_drv,
...
static int hns_mac_init_ex(struct hns_mac_cb *mac_cb)
{
	...
	hns_mac_adjust_link(mac_cb, mac_cb->speed, !mac_cb->duplex);
			     ...
			     struct device_node *np, u32 mac_mode_idx)
{
	...
	if (of_property_read_string_index(np, "media-type", mac_cb->mac_id,
					  &media_type) < 0)
		return;
...
int hns_mac_get_cfg(struct dsaf_device *dsaf_dev, int mac_idx)
{
	...
	u32 cpld_offset[DSAF_MAX_PORT_NUM_PER_CHIP];
	u32 mac_mode_idx;
	int ret;
	...
	if (dsaf_dev->cpld_base &&
		...
		/* read the offset from dts first, or use the default value */
		if (!device_property_read_u32_array(
				mac_cb->dev, "cpld_offset",
				cpld_offset, DSAF_SERVICE_PORT_NUM_PER_DSAF))
	...
	mac_cb->irq_info.virq = INVALID_VIRQ;
...
int hns_mac_init(struct dsaf_device *dsaf_dev)
{
	...
	hns_dsaf_mac_cfg_srst(dsaf_dev);
