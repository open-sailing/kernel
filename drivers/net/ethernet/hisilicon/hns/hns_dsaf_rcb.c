...
void hns_rcb_reset_ring_hw(struct hnae_queue *q)
{
	...
	while (try_cnt++ < RCB_RESET_TRY_TIMES) {
		...
		dsaf_write_dev(q, RCB_RING_TO_BE_RST, 1);
		...
		while (!could_ret && (wait_cnt < RCB_RESET_WAIT_TIMES)) {
			dsaf_write_dev(q, RCB_RING_TO_BE_RST, 0);
...
static void hns_rcb_set_port_gap_time(
	struct rcb_common_cb *rcb_common, u32 port_idx, u32 gap_time)
{
	...
	struct rcb_common_cb *rcb_common, u32 port_idx, u32 timeout)
{
	...
	} else if (rcb_common->comm_index == HNS_DSAF_COMM_SERVICE_NW_IDX) {
		if (timeout > HNS_RCB_DEF_GAP_TIME_USECS)
			hns_rcb_set_port_gap_time(rcb_common, port_idx,
						  HNS_RCB_DEF_GAP_TIME_USECS);
		else
			hns_rcb_set_port_gap_time(rcb_common,
						  port_idx, timeout);
...
int hns_rcb_common_init_hw(struct rcb_common_cb *rcb_common)
{
	...
	if (AE_IS_VER1(rcb_common->dsaf_dev->dsaf_ver)) {
		dsaf_write_dev(rcb_common, RCB_COM_CFG_FNA_REG, false);
		dsaf_write_dev(rcb_common, RCB_COM_CFG_FA_REG, true);
	} else {
		...
		dsaf_set_dev_bit(rcb_common, RCBV2_COM_CFG_TSO_MODE_REG,
				 RCB_COM_TSO_MODE_B, HNS_TSO_8BD_32K_MODE);
	}
...
static void rcb_ecc_error_int_handler(struct rcb_common_cb *rcb_common)
{
	...
	if (flag != HNS_OK_STATE) {
		dsaf_write_dev(rcb_common, RCB_COM_INTMASK_ECC_ERR_REG,
			       HNS_RCB_IRQ_DISABLE);
		if (rcb_common->dsaf_dev->ae_dev.callback_ops->event_report)
			rcb_common->dsaf_dev->ae_dev.callback_ops->event_report(
				0, 0, NIC_DEVT_NEED_PANIC,
			   ...
			   int comm_index)
{
	...
	dsaf_mode = dsaf_dev->dsaf_mode;
	ring_num = hns_rcb_get_ring_num(dsaf_dev, comm_index);

	rcb_common =
		devm_kzalloc(dsaf_dev->dev, sizeof(*rcb_common) +
			     ring_num * sizeof(struct ring_pair_cb),
			     GFP_KERNEL);
	if (!rcb_common) {
	...
	if (!AE_IS_VER1(dsaf_dev->dsaf_ver) &&
	    (comm_index == HNS_DSAF_COMM_SERVICE_NW_IDX)) {
		...
		snprintf(irq_info->irq_name, EVENT_NAME_LEN, "rcb_ring_irq");
		...
		snprintf(irq_info->irq_name, EVENT_NAME_LEN, "rcb_axi_irq");
		irq_info->irq_name[EVENT_NAME_LEN - 1] = '\0';
		irq_info->virq = virq_axi;
	} else {
		rcb_common->irq_ring_info.virq = INVALID_VIRQ;
		rcb_common->irq_axi_info.virq = INVALID_VIRQ;
	}
...
void hns_rcb_get_ring_regs(struct hnae_queue *queue, void *data)
{
	...
	regs[24] = dsaf_read_dev(queue, RCB_RING_TO_BE_RST);
