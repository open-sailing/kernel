...
#define HNSV2_RXD_VLANID_S	16
#define HNSV2_RXD_VLANID_M	(0xfff << HNSV2_RXD_VLANID_S)
...
struct hnae_ring {
	...
	/* num of bds whose buffer is not replaced
	 * successfully due to page allocation failure
	 */
	int desc_leak;
...
enum hnae_media_type {
	HNAE_MEDIA_TYPE_UNKNWON = 0,
	HNAE_MEDIA_TYPE_FIBER,
	HNAE_MEDIA_TYPE_FIBER_QSFP,
	HNAE_MEDIA_TYPE_FIBER_LCO,
	HNAE_MEDIA_TYPE_COPPER,
	HNAE_MEDIA_TYPE_BACKPLANE,
	HNAE_MEDIA_TYPE_CX4,
	HNAE_MEDIA_TYPE_VIRTUAL
};
...
struct hnae_ae_ops {
	...
	int (*reset)(struct hnae_handle *handle);
	int (*dsaf_reset)(struct hnae_handle *handle);
	...
	int (*set_uc_addr)(struct hnae_handle *handle, void *addr);
	int (*clr_uc_addr)(struct hnae_handle *handle);
	int (*add_rx_vlan)(struct hnae_handle *handle, u16 vid);
	int (*del_rx_vlan)(struct hnae_handle *handle, u16 vid);
	...
	int (*set_mdio_reg)(struct hnae_handle *handle, unsigned int page,
			    unsigned int reg, unsigned int val);
	int (*get_mdio_reg)(struct hnae_handle *handle, unsigned int page,
			    unsigned int reg, unsigned int *val);
	int (*port_irq_init)(struct hnae_handle *handle);
	void (*port_irq_free)(struct hnae_handle *handle);
	void (*config_dcb_pfc)(struct hnae_handle *handle, u8 pfc_en);
	void (*set_dcb_pfc_pause_state)(struct hnae_handle *handle,
					u8 state);
	void (*get_dcb_pfc_pause_state)(struct hnae_handle *handle,
					u8 *state);
	void (*get_dcb_up2tc)(struct hnae_handle *handle, u8 *prio_tc);
	void (*set_dcb_up2tc)(struct hnae_handle *handle, u8 *prio_tc);
	void (*get_dcb_tc_cfg)(struct hnae_handle *handle, u8 tc, u8 *state);
	void (*set_dcb_tc_cfg)(struct hnae_handle *handle, u8 tc, u8 state);
	int (*get_rst_lock)(struct hnae_handle *handle);
	void (*release_rst_lock)(struct hnae_handle *handle);
};

struct hnae_callback_ops {
	void (*event_report)(int dsaf_id, int port_id,
			     enum nic_dev_event_type event_type,
			     enum nic_device_type device_type,
...
struct hnae_ae_dev {
	...
	struct hnae_callback_ops *callback_ops;
};

enum hns_port_unavailable {
	HNS_RING_RESET_ERR = 0,
	HNS_PORT_IN_RSTING,
	HNS_PORT_ERR_MAX
};
...
struct hnae_handle {
	...
	unsigned long port_unavailable;
	rwlock_t rst_lock;
	atomic_t rst_forbid_cnt;
...
static inline struct net_device *hns_get_netdev(struct hnae_handle *handle)
{
