...
static int hns_ppe_get_cfg(struct ppe_common_cb *ppe_common)
{
	...
	pdev = container_of(dsaf_dev->dev, struct platform_device, dev);
	...
	for (i = 0; i < ppe_num; i++) {
		...
		if (!AE_IS_VER1(dsaf_dev->dsaf_ver) &&
		    (ppe_common->comm_index == HNS_DSAF_COMM_SERVICE_NW_IDX)) {
			...
			snprintf(ppe_cb->irq_info.irq_name, EVENT_NAME_LEN,
				 "ppe_irq%d", i);
			ppe_cb->irq_info.irq_name[EVENT_NAME_LEN - 1] = '\0';
		}
 ...
 * @ppe_cb: ppe device
 */
void hns_ppe_init_hw(struct hns_ppe_cb *ppe_cb)
{
	...
	if (!AE_IS_VER1(dsaf_dev->dsaf_ver)) {
		/* enable ppe vlan tag strip */
		hns_ppe_set_vlan_strip(ppe_cb, 1);
...
void hns_ppe_prepare_init(struct dsaf_device *dsaf_dev)
{
	if (!AE_IS_VER1(dsaf_dev->dsaf_ver)) {
		/* Step 1 */
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_PPE_RESET_REQ_REG, 0xff);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_PPE_COM_RESET_REQ_REG, 0x1);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_2X_RESET_REQ_REG, 0x1);

		mdelay(10);

		/* Step 2 */
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_PPE_CLK_DIS_REG, 0xff);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_PPE_COM_CLK_DIS_REG, 0x1);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_2X_CLK_DIS_REG, 0x1);

		mdelay(10);

		/* Step 3 */
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_PPE_RESET_DREQ_REG, 0xff);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_PPE_COM_RESET_DREQ_REG, 0x1);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_2X_RESET_DREQ_REG, 0x1);

		mdelay(10);

		/* Step 4 */
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_PPE_CLK_EN_REG, 0xff);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_PPE_COM_CLK_EN_REG, 0x1);
		dsaf_write_reg(dsaf_dev->sc_base,
			       DSAF_SUB_SC_RCB_2X_CLK_EN_REG, 0x1);
	}
}

static irqreturn_t ppe_channel_int_handler(int irq, void *ppe_cb)
{
	struct hns_ppe_cb *irq_ppe_cb;
	unsigned long next;
	int port_id;
	u32 int_sts;

	irq_ppe_cb = (struct hns_ppe_cb *)ppe_cb;
	port_id = irq_ppe_cb->port;
	...
	if (dsaf_get_bit(int_sts, PPE_DDR_RW_INT))
		net_edac_dev_err(irq_ppe_cb->dev, port_id,
				 "ppe_cb(%d-%u) has configure error, cause: ppe_ddr_rw.\n",
				 HNS_DSAF_COMM_SERVICE_NW_IDX, port_id);

	if (dsaf_get_bit(int_sts, PPE_TX_DROP_INT))
		net_edac_dev_err(irq_ppe_cb->dev, port_id,
				 "ppe_cb(%d-%u) has configure error, cause: ppe_tx_drop.\n",
				 HNS_DSAF_COMM_SERVICE_NW_IDX, port_id);

	if (dsaf_get_bit(int_sts, PPE_RX_SRAM_PAR_INT))
		net_edac_dev_err(irq_ppe_cb->dev, port_id,
				 "ppe_cb(%d-%u) has unrepairable error, cause: ppe_rx_sram_par.\n",
				 HNS_DSAF_COMM_SERVICE_NW_IDX, port_id);

	if (dsaf_get_bit(int_sts, PPE_TX_SRAM_PAR_INT))
		net_edac_dev_err(irq_ppe_cb->dev, port_id,
				 "ppe_cb(%d-%u) has unrepairable error, cause: ppe_tx_sram_par.\n",
				 HNS_DSAF_COMM_SERVICE_NW_IDX, port_id);
...
int hns_ppe_irq_init(struct dsaf_device *dsaf_dev, int port_id)
{
	...
	ppe_common = dsaf_dev->ppe_common[HNS_DSAF_COMM_SERVICE_NW_IDX];
...
int hns_ppe_init(struct dsaf_device *dsaf_dev)
{
	...
	for (i = 0; i < HNS_PPE_COM_NUM; i++) {
		...
		ret = hns_ppe_get_cfg(dsaf_dev->ppe_common[i]);
		if (ret)
			goto get_cfg_fail;
	...
	if (!AE_IS_VER1(dsaf_dev->dsaf_ver)) {
		...
		ret = hns_rcb_comm_irq_init(
			dsaf_dev->rcb_common[HNS_DSAF_COMM_SERVICE_NW_IDX]);
		if (ret)
			goto get_cfg_fail;
	}
