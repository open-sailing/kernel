...
static int hisi_nmi_demo_handler(void)
{
	struct timeval tv;

	do_gettimeofday(&tv);
	
	pr_info("%s: cpu %d@time:%ld\n", __func__, smp_processor_id(), tv.tv_sec);
	return 0;
}
...
/* This funciton is designed to set the nmi watchdog state by inputing
* " echo xxx  > /sys/class/cpu_panic_class/cpu_panic/cpu_status"
*
* xxx should be:
* period: set the nmi watchdog status as periodic
* oneshot: set the nmi watchdog status as oneshot
* off:	turn of the nmi watchdog function
*/
			...
			*attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "period", 6)) {
		cpu_nmi[cpu].state = NMI_WATCHDOG_PERIODIC;
		nmi_set_active_state(cpu, cpu_nmi[cpu].state);
	} else if (!strncmp(buf, "oneshot", 7)) {
		cpu_nmi[cpu].state = NMI_WATCHDOG_ONESHOT;
		nmi_set_active_state(cpu, cpu_nmi[cpu].state);
...
static int __init cpu_monitor_init(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		register_nmi_handler(cpu, hisi_nmi_demo_handler);
	}
exit:
	return ret;
}
