				    ...
				    struct device *dev)
{
	...
	for (i = 0 ; i < ARRAY_SIZE(reset_lookup_table); i++) {
		/*
		 * If the device is not import from device tree, there is no
		 * compatible property present. So lookup reset module via
		 * device name is necessary.
		 */
		if (no_compat) {
			if (!strcmp(reset_lookup_table[i].dev_name, vdev->name)) {
